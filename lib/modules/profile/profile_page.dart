...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
  create: (context) => UserCubit()..initProfilePage(uId: widget.userUid, context: context),
            ...
            return Scaffold(
                appBar: AppBar(
                  actions: [
                    IconButton(
                    onPressed: (){
                      Share.share("app://firebase-helper.com/profile/${widget.userUid}");
                    },icon: const Icon(Icons.share) , color: ConstColors.primaryColor,),
                  ],
                ),
                body: BlocProvider.value(value:UserCubit.get(context) , child: state is GetUserDataSuccessState
                    ? ProfileScreenMainPage(user:UserCubit.get(context).user)
                    : state is GetUserDataFailState?ProfileScreenFailPage(userUid: widget.userUid,):const ProfileScreenLoadingPage()),);},
        ),
);
  }

  void homePageScreenHandleStates(BuildContext context, UserState state){
    if(state is UpdateUserDataLoadingState ||state is UpdateUserImageLoadingState){
      context.pop();
    }


    if(state is UpdateUserDataSuccessState||state is UpdateUserDataFailState || state is UpdateUserImageFailState || state is UpdateUserImageSuccessState){
      UserCubit.get(context).initProfilePage(uId: widget.userUid,context: context ,);
    }
    ...
    if(state is GetUserDataFailState ){
      // showUndefinedErrorAlert(context);
    }
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              ...
              onPressed: () {
                UserCubit.get(context).initProfilePage(uId: userUid,context: context);
              },
              child: const Text(
                'Retry',
            ...
            FirebaseAuth.instance.currentUser==null?DefaultButton(text: "Sign In", onPressed: (){}) :logOutButton(context: context)
          ],
        ),
      ),
    );
      ...
      child: Padding(
        ...
        child: ListView(
          children: [
            ...
            Container(
              margin: EdgeInsets.only(top: 25.h , left:  0.5.sw - (110.w)  , right:  0.5.sw - (110.w) ),
              color: Colors.black,
              height: 26.h,
              width: 128,
            ),
